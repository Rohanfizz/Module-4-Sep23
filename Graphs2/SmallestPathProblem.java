   ...
   public static void multisolver(ArrayList<Edge>[] graph, int src, int dest, boolean[] path, int criteria, int k, String psf, int wsf) {
      ...
      //All Variables have been initialized above the function.
	   ...
	   if(src == dest){
		   ...
		   if(wsf < spathwt){//Selecting the smallest path according to weight
			   spath  = psf;
			   spathwt = wsf;
		   }
		   //We have to find a path which is having a weight just > criteria
		   if(wsf > criteria){//this path can be a potential ceil
			   if(wsf < cpathwt){
				   cpath = psf;
				   cpathwt = wsf;
			   }
